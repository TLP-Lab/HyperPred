...
from model.graph_diffusion_encoder import GraphDiffusionEncoder
from model.temporal_receptive_aggregator import TemporalReceptiveAaggregator
...
class HyperPred(nn.Module):
    def __init__(self, args):
        ...
        self.window_size = args.nb_window
        self.history_initial = torch.ones(args.num_nodes, args.nout).to(args.device)
        self.history = []
        self.spatial_diffusion_conv = GraphDiffusionEncoder(args)
        self.c_in=self.spatial_diffusion_conv.c_in
        self.c_out = self.spatial_diffusion_conv.c_out
        self.nout = args.nout
        self.temporal_casual_attention = TemporalReceptiveAaggregator(self.c_out,self.nout,self.nout,args=args)
