...
class TemporalReceptiveAaggregator(nn.Module):
    def __init__(self,k_hidden,in_channels, out_channels, args=None):
        ...
        self.receptive_aggregation = nn.ModuleList([EfficientMLHA(manifold=self.manifold,k_hidden=self.c, in_channels=self.in_channels, out_channels=self.hidden_channels,casual_kernel_size=self.casual_conv_kernel_size,dilation=self.casual_conv_kernel_size**layer,heads_ratio=1.0, dim=4, scales=self.scales)
             for layer in range(self.receptive_depth)])

    def forward(self, x):
        skips = []
        for layer in self.receptive_aggregation:
            skip, x = layer(x)
